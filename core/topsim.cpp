...
void TopSimFamily::run(int qv, int k){
    ...
    timer.start();
    switch(type){
    	case 0: topsim_sm(qv, k); break;
    	case 1: trun_topsim_sm(qv, k); break;
    	case 2: prio_topsim_sm(qv, k); break;
    	default: printf("Not supported type %d of topsim. [valid ones: 0, 1, 2.]\n", type);
    }
    timer.stop();
...
//    printf("sourceNodes:\n"); for(int i = 0; i < size; i++) { printf("%d ", sourceNodes[i]); } printf("\n");
    printf("levelCount:\n"); for(int i = 0; i < maxSteps; i++) { printf("%d ", levelNodeCount[i]); } printf("\n");
...
void TopSimFamily::getSourceNode(int qv, int depth, MyQueue* queue, int* levelNodeCount){
    ...
    while(!queue->empty() && curLevel + 1 < depth){ //only expanding level 0 to depth - 2, becasue we only need to get neighbors between 1 to depth - 1.
        int cur = queue->front();
        queue->pop();
        ...
        if(visCurLevel >= levelNodeCount[curLevel]) { visCurLevel = 0; curLevel++; ts++;}
    }
    delete [] vis;
}
...
SimMap TopSimFamily::trun_topsim_sm_computation(int qv, int pathLen, int* sourceNodes, int size, int* levelNodeCount, int highDegree, double eta){
	int processCnt = 0;
	int curLevel = pathLen - 1;
	vector<SimMap> SMList;
	vector<SimMap> mergeMap;
...
SimMap TopSimFamily::prio_topsim_sm_computation(int qv, int pathLen, int* sourceNodes, int size, int* levelNodeCount, int priority){
	long processCnt = 0;
	int curLevel = pathLen - 1;
	int maxMapSize = -1;
    int levelVis = 0;
