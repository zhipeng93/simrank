...
SRGS::query(int qv, int k){
    ...
    tt.stop();
	printf("Cost for transferring: %.5lf %d\n", tt.getElapsedTime(), sim2.size());
...
void
SRGS::updateSimrank(vector<SimRankValue>& sim, int** randwalk, int sid, int qv){
		...
		for(int sid = 0; sid < sampleNum; ++sid){
			for(int i = 0; i < maxVertexId; ++i){
				...
				if(graphSrc[i] != graphSrc[i+1]){
					int idx = rnum % (graphSrc[i+1] - graphSrc[i]);
					gsm->insertEdge(sid, i, graphDst[graphSrc[i] + idx]);
//	                printf("sid=%d sampled edge in initialzation: (%d, %d) idx=%d\n",sid, i, graphDst[graphSrc[i] + idx], idx);
				}
			}
		}
        if(uDisk == 1)
		    ((DiskRGSManager*)gsm)->save(sampleNum - 1);
        if(uDisk == 2)
		    ((CompressDiskRGSManager*)gsm)->save(sampleNum - 1);
        if(uDisk == 3)
        	((DynamicOwgManager*)gsm)->save(sampleNum - 1);
	}
*/
}
