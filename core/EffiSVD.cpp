...
void EffiSVD::initialize(){
    ...
    */
	...
	for(int i=0;i<maxVertexId;i++)
		for(int j=0;j<maxVertexId;j++)
		{
			X(i,j)=cx_double(1 - decayFactor, 0.0) * p_t_p(i,j) / eigval(i) / ( cx_double(1,0) / eigval(i) - cx_double(decayFactor,0) * eigval(j) );		
		}
