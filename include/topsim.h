...
class TopSimFamily : public SimRankMethod{
	public:
        ...
        int* orig_graph_src, int* orig_graph_dst, int mvid, int type_) :
		    ...
		    graphSrc(graph_src), graphDst(graph_dst), orig_graphSrc(orig_graph_src), orig_graphDst(orig_graph_dst), maxVertexId(mvid) { }
	...
	private:
		...
		SimMap trun_topsim_sm_computation(int qv, int pathLen, int* sourceNodes, int size, int* levelNodeCount, int highDegree, double eta);
		SimMap prio_topsim_sm_computation(int qv, int pathLen, int* sourceNodes, int size, int* levelNodeCount, int priority);
        ...
        void visit(int vid, bool* vis, int& count) { if(vis[vid] == false) {count++; vis[vid] = true;}}
